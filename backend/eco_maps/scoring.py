...
def score_route(route: Dict, context: Dict, config: Dict, 
    ...
    Returns:
        Dictionary with detailed scoring breakdown (without eco_score - added by rank_routes)
    """
    ...
    base_wh_per_km = energy_models.get(mode, 150)  # Default 150 Wh/km for EV
    ...
    # Calculate raw weighted score (lower is better)
    # This will be normalized against other routes in rank_routes()
    raw_score = (
        time_weight * eta_minutes +
        co2_weight * co2_kg * 10 +  # Scale CO2 to be comparable
    ...
    return {
        ...
        'raw_score': raw_score  # Used for ranking
    }
...
def rank_routes(routes: List[Dict], context: Dict, config: Dict,
    ...
    Eco scores are normalized relative to all routes (best=10, worst=0).
    ...
    raw_scores = [r['raw_score'] for r in scored_routes]
    min_score = min(raw_scores)
    max_score = max(raw_scores)
    
    # Normalize to 0-10 scale (lower raw score = better = higher eco_score)
    for route in scored_routes:
        if max_score == min_score:
            # All routes identical
            route['eco_score'] = 10.0
        else:
            # Invert: best (lowest raw score) gets 10, worst gets 0
            normalized = (max_score - route['raw_score']) / (max_score - min_score)
            route['eco_score'] = round(normalized * 10, 2)
